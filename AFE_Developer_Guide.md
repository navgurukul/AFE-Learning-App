...
```mermaid
graph TD
    Desktop["apps/desktop<br/>(Electron Main)"]
    Renderer["apps/renderer<br/>(React UI)"]
    Shared["@afe/shared<br/>(Types + Contracts)"]
    DB["@backend/db<br/>(SQLite + Drizzle)"]
    ContentEngine["@backend/content-engine<br/>(Manifest Loader)"]
    AITutor["@backend/ai-tutor<br/>(Ollama LLM)"]
    Analytics["@backend/analytics<br/>(Events + Sync)"]
    STT["@backend/stt-engine<br/>(Whisper.cpp)"]
    TTS["@backend/tts-engine<br/>(Piper ONNX)"]
...
graph TB
    subgraph "Renderer Process (Sandboxed Chrome)"
        ...
        IPCClient["IPC Client<br/>(lib/ipc.ts)"]
    end

    subgraph "Security Layer"
        Preload["Preload Script<br/>(secure.cjs)<br/>Channel Whitelist"]
    end

    subgraph "Main Process (Node.js)"
        Handlers["IPC Handlers<br/>(handlers.ts)"]
        
        subgraph "Backend Services"
            DB_SVC["Database Service<br/>(better-sqlite3 / Drizzle)"]
            CE_SVC["Content Engine<br/>(Zod Manifest)"]
            AI_SVC["AI Tutor<br/>(Ollama qwen2.5:1.5b)"]
            AN_SVC["Analytics<br/>(Events + Sync)"]
            STT_SVC["STT Engine<br/>(Whisper.cpp)"]
            TTS_SVC["TTS Engine<br/>(Piper ONNX)"]
        end
        ...
        subgraph "System Services"
            ...
            DeviceInfo["Device Info<br/>(Serial, MAC, NGO Key)"]
        end
    end
    ...
    subgraph "External"
        ...
        Manifest[("Content Manifest<br/>manifest.json")]
        Assets[("Video/PDF Assets")]
        RMS["RMS Server<br/>(Cloud)"]
    end
...
```mermaid
graph LR
    A["/ <br/> BeginLearning"] --> B["/avatar-selection <br/> AvatarSelection"]
    A --> C["/modules/:studentId <br/> ModuleList"]
    C --> D["/module/:studentId/:moduleId <br/> ModuleDetail"]
    C --> E["/dashboard/:studentId <br/> StudentDashboard"]
    C --> F["/ai-tutor/:studentId <br/> AILearningCenter"]
...
```mermaid
flowchart LR
    User["Student<br/>(Renderer)"] -->|Message| IPC["IPC Handler"]
    IPC -->|"sendMessage()<br/>or sendVoiceMessage()"| Service["AI Tutor Service"]
    
    Service -->|"Build context"| DB["Database<br/>(Session + History)"]
    Service -->|"Build prompt"| Prompts["Prompt Builder<br/>(prompts.ts)"]
    Service -->|"Stream chat"| Ollama["Ollama<br/>localhost:11434<br/>qwen2.5:1.5b"]
    ...
    Service -->|"After 5s delay"| Title["Title Generator<br/>qwen2.5:0.5b"]
...
sequenceDiagram
    ...
    DS->>DS: Generate missing date range → yesterday
    
    loop For each student × each date
        DS->>DS: calculateMetricsForDate()
        Note over DS: SUM video time, read time,<br/>AVG quiz scores, COUNT modules
    ...
    Note over DS: Gap-filling: if device was<br/>offline for 5 days, creates<br/>5 snapshots on next startup
```
...
```mermaid
flowchart LR
    Mic["Browser MediaStream<br/>(16kHz PCM)"] -->|"stt:chunk"| Preload
    Preload -->|"Raw PCM ArrayBuffer"| Handler["IPC Handler<br/>(stt:chunk)"]
    Handler -->|"pushAudioChunk()"| Buffer["Audio Buffer<br/>(in-memory)"]
    
    User["User releases<br/>push-to-talk"] -->|"stt:stop"| Stop["processAudio()"]
    Stop --> Concat["Concatenate chunks"]
    Concat --> WAV["Create WAV header<br/>(44 bytes + PCM)"]
    WAV --> Temp["Write temp .wav file"]
    Temp --> Whisper["whisper-cli<br/>-m ggml-base-q5_1.bin<br/>--no-timestamps<br/>--language auto"]
    Whisper --> Clean["Clean output<br/>(remove tags)"]
...
flowchart LR
    subgraph "Installer (Build Time)"
        Manifest["manifest.json<br/>(modules + lessons)"]
    ...
    subgraph "Runtime"
        ...
        DB[("SQLite<br/>modules + lessons")]
        Media["media:// protocol<br/>(video/PDF serving)"]
    end
...
flowchart TD
    ...
    subgraph "Event Tracking"
        Track["trackEvent()"]
        Events[("analytics_events<br/>(append-only)")]
    end
